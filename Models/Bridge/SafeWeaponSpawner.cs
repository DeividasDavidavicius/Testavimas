...
{
    public class SafeWeaponSpawner : IWeaponSpawner
    {
        public void Spawn(Color color, int damage, Grid grid, GameSession session)
        {
            ...
            for (int i = 0; i < grid.gridSize; i++)
            {
                for (int j = 0; j < grid.gridSize; j++)
                {
                    ...
                    if (cell.GetType() == typeof(EmptyCell) && leftCell.GetType() == typeof(EmptyCell) && upCell.GetType() == typeof(EmptyCell)
                        && rightCell.GetType() == typeof(EmptyCell) && downCell.GetType() == typeof(EmptyCell))
                    {
                        emptyCellsX.Add(i);
                        emptyCellsY.Add(j);
